...
const obj={
  openapi: '3.0.3',
  info: { title: 'F-Grade Zoho CRM API', version: '1.0.0', description: 'Read-only CRM endpoints (generated).' },
  servers: [ { url: 'https://unweighted-elois-trinomially.ngrok-free.dev' } ],
  paths: {
    '/api/crm/leads': { get: { operationId: 'getLeads', summary: 'Get Leads', parameters: [ { name: 'page', in: 'query', schema: { type: 'integer' } }, { name: 'per_page', in: 'query', schema: { type: 'integer' } } ], responses: { '200': { description: 'Successful CRM response', content: { 'application/json': { schema: { $ref: '#/components/schemas/CRMResponse' } } } }, '400': { description: 'Bad Request' }, '500': { description: 'Server Error' } } } },
...
fs.writeFileSync('./src/crm/openapi/crm.openapi.json', JSON.stringify(obj, null, 2), 'utf8');
console.log('WROTE');
